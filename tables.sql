...
CREATE TABLE IF NOT EXISTS index_tag (
 id		INT(10) NOT NULL AUTO_INCREMENT,
 name		VARCHAR(255) NOT NULL,
 tag_sequence	VARCHAR(255) NOT NULL,	
...
CREATE TABLE IF NOT EXISTS experiment (
 ...
 spike_dilution				VARCHAR(255) DEFAULT "No dilution" NOT NULL,
 ...
 sample_visability			ENUM('Hold', 'Public') DEFAULT 'Hold' NOT NULL,	 
...
CREATE TABLE IF NOT EXISTS rna_dilution_plate (
 ...
 rna_extraction_id			INT(10) NOT NULL,
...
CREATE OR REPLACE VIEW smpView AS
 SELECT smp.id ID, 
        ...
        indt.name Tag_name, 
        indt.tag_sequence Tag_sequence, 
        smp.embryo_collection_well_number Embryo_collection_well_number, 
        rnas.well_number RNA_dilution_well_number,
 ...
 FROM sample smp INNER JOIN experiment exp 
        ...
        ON rnas.index_tag_id = indt.id
 GROUP BY smp.id;


CREATE OR REPLACE VIEW get_smpView AS
SELECT  exp.id Experiment_id,
        GROUP_CONCAT(rna_dlip.well_number SEPARATOR " : ") Sample_well_order,
 ...
 FROM experiment exp LEFT OUTER JOIN sample smp 
        ...
        ON rna_dlip.index_tag_id = ind_tag.id
 GROUP BY rna_dlip.well_number;
...
CREATE PROCEDURE add_experiment_data(
 ...
 IN spike_dilution_param INT(10),
